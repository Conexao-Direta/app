...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  const decoded = verify(token, authConfig.secret);
  const { companyId } = decoded as TokenPayload;
  const { name, baseUrl, model, apiKey, prompt, maxTokens, temperature, promptTokens, completionTokens, totalTokens, queueId, maxMessages,voice,voiceKey,voiceRegion } = req.body;
  const promptTable = await CreatePromptService({ name, baseUrl, model, apiKey, prompt, maxTokens, temperature, promptTokens, completionTokens, totalTokens, queueId, maxMessages, companyId,voice,voiceKey,voiceRegion });
